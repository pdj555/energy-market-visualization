...
const mockSnapshot = {
  ...
  insights: {
    ...
    alerts: ['Price spike detected: +4.1% vs average'],
  },
};
...
describe('DashboardView', () => {
  ...
  it('renders dashboard with market data', async () => {
    renderDashboard();

    expect(await screen.findByText('Energy Intelligence')).toBeInTheDocument();
    expect(await screen.findByText('five ISO regions · synthetic telemetry')).toBeInTheDocument();

    const chart = await screen.findByTestId('mock-line-chart');
    expect(chart).toHaveAttribute('data-point-count', '5');

    expect(await screen.findByRole('columnheader', { name: 'Mid' })).toBeInTheDocument();
