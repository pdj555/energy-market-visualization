      ...
      <div className='lg:col-span-2'>
        <div className='mb-4 flex flex-col gap-1'>
          ...
          <p className='text-xs text-slate-400'>
            Sampling window {formatDateTime(insights.windowStart)} →{' '}
            {formatDateTime(insights.windowEnd)}
          </p>
        </div>
            ...
            <ul className='space-y-3 text-sm text-slate-200'>
              {insights.alerts.map(alert => (
                <li key={alert} className='flex items-start gap-3'>
                  <span
                    className='mt-1 inline-flex h-2.5 w-2.5 flex-none rounded-full bg-amber-400'
                    aria-hidden='true'
                  />
                  <span>{alert}</span>
                </li>
