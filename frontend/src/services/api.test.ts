...
describe('ApiService', () => {
  ...
  describe('getCurrentPrices', () => {
    it('fetches current prices successfully', async () => {
      const mockPrices = [
        { energyType: EnergyType.ELECTRICITY, price: 45.50, unit: 'MWh', timestamp: '2024-01-15T10:30:00Z', changePercent: 2.5 },
        { energyType: EnergyType.GAS, price: 30.25, unit: 'MMBtu', timestamp: '2024-01-15T10:30:00Z', changePercent: -1.2 },
      ];
  ...
  describe('getPriceByType', () => {
    it('fetches price for specific energy type', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:8080/api/energy-market/prices/ELECTRICITY');
  ...
  describe('getEnergyTypes', () => {
    it('fetches energy types successfully', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('http://localhost:8080/api/energy-market/energy-types');
